...
Chances are that one of the domain in that list is associated to the same account and it's in the CrimeFlare database.
...
With some basic PHP lines and a small `.htcaccess` modification, it's indeed possible to create a link to an image that logs the IP address of anyone who tries to retrieve the resource:
```
